...
def stark_key_message(chainId: int):
    message = {
        "domain": {"name": "linora", "version": "1", "chainId": chainId},
        "primaryType": "Constant",
        "types": {
            "EIP712Domain": [
...
def get_recovery_phrase_dict(config: dict) -> str:
    if config.get("linora_environment") and config.get("linora_environment") == "local":
...
def generate_accounts_dict(config: dict) -> dict:
    ...
    eth_chain_id = int(config.get("linora_config", {}).get("l1_chain_id"))
    ...
    config["linora_account"] = get_acc_contract_address_and_call_data(
        proxy_class_hash,
        account_class_hash,
        hex(key_pair.public_key),
    )
    logging.info(f"{FN} config.linora_account: {config['linora_account']}")
    return config

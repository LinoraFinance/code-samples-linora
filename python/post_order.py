...
from shared.api_client import get_jwt_token, get_linora_config, post_order_payload, sign_order
...
async def main(config: ApiConfig) -> None:
    ...
    config.linora_account, config.linora_account_private_key = generate_linora_account(
        config.linora_config, eth_account.key.hex()
    )

    # Get a JWT token to interact with private endpoints
    logging.info("Getting JWT...")
    linora_jwt = await get_jwt_token(
        config.linora_config,
        config.linora_http_url,
        config.linora_account,
        config.linora_account_private_key,
    )

    # POST order
    order = build_order(config, OrderType.Market, OrderSide.Buy, Decimal("0.1"), "ETH-USD-PERP", "mock")
    await post_order_payload(config.linora_http_url, linora_jwt, order.dump_to_dict())
